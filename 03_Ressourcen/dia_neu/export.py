...
SPALTEN_NAMEN_ORIG = ["5% In", "20% In", "50% In", "80% In", "90% In", "100% In", "120% In"]
...
def calculate_layout_adjustments(data, base_height, ncol=2):
    if "hersteller" in data.columns:
        unique_items = len(data[["hersteller", "modell", "technologie", "geometrie"]].drop_duplicates())
    else:
        unique_items = 1
...
def assign_dynamic_colors(df):
    ...
    unique_entries = df[["hersteller", "modell", "technologie", "_color_key"]].drop_duplicates()
    color_map = {}
    
    groups = {
        "mbs": [], "celsa_std": [], "celsa_komp": [], "redur": [], 
        "siemens": [], "3-K Elektrik": [], "rogowski": [], "other": []
    }
    ...
    for _, row in unique_entries.iterrows():
        ...
        if "mbs" in h: groups["mbs"].append(key)
        elif "celsa" in h:
            if "kompensiert" in t: groups["celsa_komp"].append(key)
            else: groups["celsa_std"].append(key)
        elif "redur" in h or "ffp" in t: groups["redur"].append(key)
        elif "siemens" in h or "pac" in m: groups["siemens"].append(key)
        elif "k-3" in h or "3-K Elektrik" in h: groups["3-K Elektrik"].append(key)
        elif "rogowski" in h: groups["rogowski"].append(key)
        else: groups["other"].append(key)

    for k, c in zip(sorted(groups["mbs"]), generate_color_palette(COLOR_MBS_BASE, len(groups["mbs"]))): color_map[k] = c
    for k, c in zip(sorted(groups["celsa_std"]), generate_color_palette(COLOR_CELSA_BASE, len(groups["celsa_std"]))): color_map[k] = c
    for k, c in zip(sorted(groups["celsa_komp"]), generate_color_palette(COLOR_CELSA_KOMP, len(groups["celsa_komp"]))): color_map[k] = c
    for k, c in zip(sorted(groups["redur"]), generate_color_palette(COLOR_REDUR_BASE, len(groups["redur"]))): color_map[k] = c
    for k, c in zip(sorted(groups["siemens"]), generate_color_palette(COLOR_SIEMENS, len(groups["siemens"]))): color_map[k] = c
    for k, c in zip(sorted(groups["3-K Elektrik"]), generate_color_palette(COLOR_3K, len(groups["3-K Elektrik"]))): color_map[k] = c
    for k, c in zip(sorted(groups["rogowski"]), generate_color_palette(COLOR_ROGOWSKI, len(groups["rogowski"]))): color_map[k] = c
    for k, c in zip(sorted(groups["other"]), generate_color_palette(COLOR_GRAY_BASE, len(groups["other"]))): color_map[k] = c
...
def format_value(val):
    ...
    if val == 0: return "0"
    abs_val = abs(val)
    if abs_val < 0.1: return "< 0.1"
    if abs_val >= 10: return f"{val:.0f}"
...
def create_dynamic_legend_handles(data, color_col="color", show_geo=True):
    legend_dict = {}
    if "hersteller" in data.columns:
        data_sorted = data.sort_values(by=["hersteller", "modell", "technologie", "geometrie"])
    else:
        data_sorted = data
    ...
    for _, row in data_sorted.iterrows():
        ...
        label = f"{hersteller} {modell} ({tech})"
        if show_geo:
            label = f"{label} [{geo_suffix}]"

        if label not in legend_dict:
            legend_dict[label] = Patch(facecolor=color, hatch=hatch, edgecolor='black', label=label)
...
def plot_range_analysis(data, filename, folder_key, group_name):
    ...
    rects1 = ax.barh(y_pos + height, df_agg['range_low'], height, label='Niederstrom (5-50%)', color=c_low, edgecolor='black')
    rects2 = ax.barh(y_pos, df_agg['range_nom'], height, label='Nennstrom (80-100%)', color=c_nom, edgecolor='black')
    rects3 = ax.barh(y_pos - height, df_agg['range_high'], height, label='Überlast (120%)', color=c_high, edgecolor='black')
    ...
    ax.set_title("Fehleranalyse nach Lastbereich", fontsize=FONT_TITLE, fontweight='bold', pad=20)
    
    def label_bars(rects):
        for rect in rects:
            width = rect.get_width()
            if width > 0:
                ax.text(width + (width*0.01), rect.get_y() + rect.get_height()/2, 
    ...
    all_vals = np.concatenate([df_agg['range_low'], df_agg['range_nom'], df_agg['range_high']])
    if len(all_vals) > 0:
...
def plot_unified_bars(data, x_col, y_col, color_col, title, ylabel, filename, folder_key, group_name, sort_col=None):
    if data.empty: return
    ...
    try:
        groups = sorted(groups, key=lambda x: float(x) if str(x).replace('.','',1).isdigit() else x)
    except:
        pass
    ...
    calc_height, calc_bottom = calculate_layout_adjustments(data, BASE_HEIGHT_BAR, ncol=leg_ncol)
    ...
    if max_val > 0: ax.set_ylim(top=max_val * 1.15)
    ...
    for i, group_val in enumerate(groups):
        ...
        for j in range(n_bars):
            ...
            if pd.notnull(val):
                ...
                ax.bar(x_pos, val, width=draw_width, color=color, edgecolor="black", linewidth=1.5)
    ...
    handles = create_dynamic_legend_handles(data, color_col, show_geo=show_geo_in_legend)
    if handles:
...
def plot_horizontal_generic(data, value_col, title, x_label, filename, folder_key, group_name, is_cost=False):
    if data.empty: return

    sort_cols = [c for c in ["nennstrom", "hersteller", "modell", "geometrie"] if c in data.columns]
    if sort_cols:
        if "nennstrom" in data.columns:
            data["_ns_sort"] = pd.to_numeric(data["nennstrom"], errors="coerce").fillna(0)
    ...
    calc_total_h, calc_bottom = calculate_layout_adjustments(data, data_height, ncol=leg_ncol)
    ...
    for bar, val in zip(bars, values):
        ...
        ax.text(text_x, bar.get_y() + bar.get_height()/2, label_text, va='center', fontsize=14, fontweight='bold')
...
def draw_limit_lines(ax, accuracy_class="1.0"):
    ...
    ax.plot(x_lims, y_upper, color='black', linestyle='--', linewidth=2.5, alpha=0.8, label=label_text)
...
def plot_line_curves_thesis_grouped(data, group_name):
    ...
    print(f"  -> Plot Verlauf für Gruppe: {group_name} ({current_val} A) [Kl. {acc_class}]")

    calc_height, calc_bottom = calculate_layout_adjustments(data, BASE_HEIGHT_LINE, ncol=leg_ncol)

    fig, axes = plt.subplots(1, 3, figsize=(20, calc_height), sharey=True)
    fig.suptitle(f"Genauigkeitsverlauf | {current_val} A", fontsize=FONT_TITLE, fontweight='bold', y=0.96)
    ...
    for i, ax in enumerate(axes):
        phase_name = phases[i]
        if phase_col:
            df_phase = data[data[phase_col].astype(str).str.contains(phase_name, case=False, na=False)]
        else:
            df_phase = data
        ...
        for _, row in df_phase.iterrows():
            ...
            # Label im Plot ohne Geo für Messstrecke
            if is_messstrecke:
                label_txt = f"{hersteller} {modell} ({tech})"
            else:
                label_txt = f"{hersteller} {modell} ({tech}) [{geo}]"

            ax.plot(x_plot, y_plot, marker=marker, markersize=MARKER_SIZE,
                    linestyle=linestyle, linewidth=LINE_WIDTH, color=color, label=label_txt)
    ...
    unique_items = {}
    for h, l in zip(handles, labels): unique_items[l] = h

    if is_messstrecke:
        # Messstrecke: Einfache Liste ohne Grenzwert
        final_handles = []
        final_labels = []
        ...
        for k in sorted_keys:
            if "grenzwert" in k.lower(): continue # Grenzwert ignorieren
            final_handles.append(unique_items[k])
            final_labels.append(k)
            
    else:
        # Standard: Split Parallel/Dreieck
        parallel_list = []
        dreieck_list = []
        for lbl, hdl in unique_items.items():
            if "grenzwert" in lbl.lower(): continue
    ...
    if final_handles:
        ...
        fig.legend(final_handles, final_labels, loc='center', ncol=leg_ncol,
                   title=legend_title, title_fontsize=FONT_LEGEND_LINE+2,
                   fontsize=FONT_LEGEND_LINE, bbox_to_anchor=(0.5, 0.12), frameon=True)
...
def lade_und_plotte_alle():
    ...
    meta_cols = ["hersteller", "modell", "technologie", "geometrie", "nennstrom", "phase", "export_group"]
    for c in meta_cols:
    ...
    preis_col = "preis (€)" 
    if preis_col in df.columns: df[preis_col] = pd.to_numeric(df[preis_col], errors="coerce")
    ...
    for group in unique_groups:
        ...
        # 2. BEREICHS-ANALYSE
        plot_range_analysis(df_group, filename=f"{group}_bereichs_analyse.png", folder_key="bereich", group_name=group)
        ...
        if not groupby_keys: continue

        agg_dict = {"phase_mean_val": "mean", "color": "first"}
        if preis_col in df_group.columns: agg_dict[preis_col] = "first"
        ...
        plot_horizontal_generic(df_agg, value_col="total_error",
                                ...
                                filename=f"{group}_wahre_fehler_horizontal.png",
                                folder_key="absolut", group_name=group, is_cost=False)

        # 5. KOSTEN
        if preis_col in df_agg.columns:
            plot_horizontal_generic(df_agg, value_col=preis_col,
                                    title="Kostenübersicht",
                                    x_label="Kosten [€]",
                                    filename=f"{group}_kosten_horizontal.png",
                                    folder_key="kosten", group_name=group, is_cost=True)
        ...
        pivot_index = [c for c in groupby_keys if c != "geometrie"] + ["color", "_sort_idx"]
        pivot_vals = ["total_error"]
        if preis_col in df_agg.columns: pivot_vals.append(preis_col)

        if "geometrie" in df_agg.columns and not df_agg.empty:
            try:
                df_pivot = df_agg.pivot(index=pivot_index, columns="geometrie", values=pivot_vals).reset_index()
                df_pivot.columns = [f"{c[0]}_{c[1]}" if c[1] else c[0] for c in df_pivot.columns]
                
                col_err_par = next((c for c in df_pivot.columns if "total_error" in c and "parallel" in c.lower()), None)
                col_err_tri = next((c for c in df_pivot.columns if "total_error" in c and "dreieck" in c.lower()), None)

                if col_err_par and col_err_tri:
                    df_pivot["imp_pct"] = (1 - (df_pivot[col_err_tri] / df_pivot[col_err_par])) * 100
                    
                    plot_unified_bars(df_pivot, x_col="nennstrom", y_col="imp_pct", color_col="color",
                                      title="Verbesserung Dreieck (%)", ylabel="Verbesserung [%]",
                                      filename=f"{group}_verbesserung_pct.png", folder_key="verbesserung", group_name=group)
                    
                    col_cost_par = next((c for c in df_pivot.columns if "preis" in c and "parallel" in c.lower()), None)
                    if col_cost_par:
                        df_pivot["imp_pct_eur"] = df_pivot["imp_pct"] / df_pivot[col_cost_par]
                        plot_unified_bars(df_pivot, x_col="nennstrom", y_col="imp_pct_eur", color_col="color",
                                          title="Wirtschaftlichkeit (% / €)", ylabel="Verbesserung [% / €]",
                                          filename=f"{group}_wirtschaftlichkeit_eur.png", folder_key="wirtschaft", group_name=group)
            except Exception as e:
                pass
