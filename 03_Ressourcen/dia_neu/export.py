...
def lade_und_plotte_alle():
    ...
    for group in unique_groups:
        ...
        idx_candidates = groupby_keys + ["color", "_sort_idx"]
        merge_keys = [
            c for c in idx_candidates if c != "geometrie" and c in df_agg.columns
        ]
        merge_keys = list(dict.fromkeys(merge_keys))

        val_cols = ["total_error"]
        if preis_col in df_agg.columns:
            val_cols.append(preis_col)

        if "geometrie" in df_agg.columns and not df_agg.empty:
            try:
                df_agg["_geo_norm"] = (
                    df_agg["geometrie"].astype(str).str.lower().str.strip()
                )
                df_par = df_agg[df_agg["_geo_norm"].str.contains("parallel")].copy()
                df_tri = df_agg[df_agg["_geo_norm"].str.contains("dreieck")].copy()

                if df_par.empty or df_tri.empty:
                    pass
                else:
                    rename_par = {c: f"{c}_parallel" for c in val_cols}
                    rename_tri = {c: f"{c}_dreieck" for c in val_cols}

                    df_par.rename(columns=rename_par, inplace=True)
                    df_tri.rename(columns=rename_tri, inplace=True)

                    cols_par = merge_keys + list(rename_par.values())
                    cols_tri = merge_keys + list(rename_tri.values())

                    df_pivot = pd.merge(
                        df_par[cols_par], df_tri[cols_tri], on=merge_keys, how="inner"
                    )

                    col_err_par = "total_error_parallel"
                    col_err_tri = "total_error_dreieck"

                    if (
                        not df_pivot.empty
                        and col_err_par in df_pivot.columns
                        and col_err_tri in df_pivot.columns
                    ):
                        df_pivot["imp_pct"] = df_pivot.apply(
                            lambda r: (
                                (1 - (r[col_err_tri] / r[col_err_par])) * 100
                                if r[col_err_par] != 0
                                else 0
                            ),
                            axis=1,
                        ...
                        plot_unified_bars(
                            ...
                            y_col="imp_pct",
                            color_col="color",
                            title=f"Verbesserung durch Dreieck-Anordnung – {prettify_group_name(group)}",
                            ylabel="Verbesserung [%]",
                        ...
                        if col_cost_par:
                            df_pivot["imp_pct_eur"] = df_pivot.apply(
                                lambda r: (
                                    r["imp_pct"] / r[col_cost_par]
                                    if r[col_cost_par] != 0
                                    else 0
                                ),
                                axis=1,
                            ...
                            plot_unified_bars(
                                ...
                                y_col="imp_pct_eur",
                                color_col="color",
                                title=f"Wirtschaftlichkeit – {prettify_group_name(group)}",
                                ylabel="Verbesserung [% / €]",
                ...
                print(f"Fehler bei Merge/Plot für Gruppe {group}: {e}")
