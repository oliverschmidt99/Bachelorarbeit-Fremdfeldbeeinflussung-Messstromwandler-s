...
Eine detaillierte Betrachtung der Schienenanordnung erfolgt in Abschnitt~\ref{sec:geometrie_kupferschienen}. Dieser Primärleiter wird durch die Fensteröffnung des Wandlers geführt.
...
Das Linienintegral der magnetischen Feldstärke \gls{sym:H} entlang eines geschlossenen Weges $\mathcal{S}$ entspricht dabei der umschlossenen Durchflutung \gls{sym:Theta}:
...
Das Wegintegral entspricht somit dem Produkt aus $H$ und dem Kreisumfang $2\pi r$:
...
Mit der magnetischen Feldkonstante \gls{sym:mu0} und der relativen Permeabilität \gls{sym:mur} folgt:
...
Der Fluss berechnet sich durch Integration der Flussdichte über die Querschnittsfläche:
...
Der Term, der die Geometrie und die Materialeigenschaften des Kerns beschreibt, stellt den magnetischen Widerstand \gls{sym:Rm} dar:
...
Die magnetische Durchflutung $\Theta$ entspricht dabei der elektrischen Spannung und der magnetische Fluss dem elektrischen Strom:
...
Die resultierende magnetische Durchflutung $\Theta_{res}$, die effektiv den Fluss im Kern treibt, ist somit die Differenz aus Primär- und Sekundärdurchflutung:
...
Setzt man die resultierende Durchflutung in das Hopkinsonsche Gesetz ein, erhält man:
...
Es stellt sich ein fast ideales Durchflutungsgleichgewicht ein:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{03_Ressourcen/zeichnungen/kupferschienensystem_phasenmittelabstand.drawio.pdf}
