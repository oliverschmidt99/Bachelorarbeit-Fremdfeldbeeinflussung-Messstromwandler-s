...
\subsubsubsection{Rogowskispulen}
\label{sec:Rogowskispulen}
...
\begin{equation}
    \oint_{\mathcal{S}} \vec{H} \cdot \mathrm{d}\vec{s} = I_{p} = \Theta
    \label{eq:ampere_integral}
\end{equation}

Der Stromwandler nutzt einen Ringkern aus ferromagnetischem Material (siehe Abschitt \ref{sec:magnetische-stoffeigenschaften}) mit einer hohen relativen Permeabilität $\mu_r \gg 1$.
...
Die magnetische Durchflutung $\Theta$ entspricht dabei der elektrischen Spannung und der magnetische Fluss dem elektrischen Strom, analog zu Gleichung \eqref{eq:hopkinson} \cite{Harriehausen2025}[Kap.~11.9]
...
Die im $B$-$H$-Diagramm eingeschlossene Fläche der Hystereseschleife ist ein Maß für die magnetischen Verluste pro Zyklus~\cite{Harriehausen2025}[Kap.~11.12.1].
...
Bereits geringe Abweichungen von der zentrischen Lage führen zu einer inhomogenen magnetischen Feld- bzw. Flussverteilung im Eisenkern und erhöhen damit die Wahrscheinlichkeit lokaler Sättigungsbereiche.
...
Abbildung \ref{pic:sim_exzentrische_moniert} zeigt den simulierten Verlauf des magnetischen Flusses entlang des mittleren Kernumfangs (X-Achse: Weg/''Länge'' in mm, Y-Achse: magnetischer Fluss in Wb).
Der blaue Kurvenverlauf (symmetrisch) entspricht der Referenzkonfiguration mit zentrierter Leiteranordnung: Der Fluss ist über den Umfang weitgehend homogen und weist eine nahezu symmetrische Verteilung auf.
Der orangefarbene Verlauf (unsymmetrisch) beschreibt die exzentrische Positionierung: In den Umfangsbereichen, in denen der Primärleiter näher am Kern liegt, steigt der magnetische Fluss deutlich an, während er auf der gegenüberliegenden Seite entsprechend abnimmt.
Diese Asymmetrie ist ein unmittelbarer Hinweis auf eine ungleichmäßige magnetische Beanspruchung des Kerns und damit auf ein erhöhtes Risiko lokaler Sättigung.

Ist die Sekundärwicklung zusätzlich nicht gleichmäßig um den Kern verteilt, kann die resultierende unsymmetrische Flussverteilung zu systematischen Messabweichungen führen, da einzelne Wicklungsabschnitte eine unterschiedliche magnetische Kopplung erfahren.
