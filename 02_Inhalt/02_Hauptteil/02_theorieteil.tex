...
Auf dieser Basis werden die maßgeblichen physikalischen Zusammenhänge entwickelt, die das Übertragungsverhalten und die Beeinflussung durch magnetische Störfelder prägen. Eine präzise mathematische Modellierung ist erforderlich, um Sättigungserscheinungen im Eisenkern und die daraus entstehenden Messfehler gezielt zu untersuchen.
...
Ein Stromwandler transformiert hohe Wechselströme aus dem Primärnetz in kleine und messbare Ströme auf der Sekundärseite. Er fungiert als Bindeglied zwischen dem Hochstrombereich und den Mess- oder Schutzeinrichtungen. Das grundlegende Funktionsprinzip beruht auf dem transformatorischen Prinzip der elektromagnetischen Induktion. Dies ermöglicht den Anschluss standardisierter Messgeräte, Zähler oder Schutzrelais für Nennströme von \SI{1,00}{\ampere} oder \SI{5,00}{\ampere}, ohne diese dem hohen Potenzial oder den hohen Strömen des Primärleiters auszusetzen.
...
Im Niederspannungsbereich fungiert meist eine Kupferschienenanordnung als Primärleiter. Diese weist in der Regel ein Rechteckprofil auf und kann aus mehreren Einzelschienen bestehen. Eine ausführliche Analyse der Schienenanordnung findet sich in Abschnitt~\ref{sec:layout_geometrie}. Der Primärleiter wird durch die Fensteröffnung des Wandlers geführt.

Die Bauform ohne integrierte Primärwicklung wird als Durchsteck- oder Aufsteckstromwandler bezeichnet. Sie entspricht physikalisch einer Windungszahl von \num{1,00} (\gls{sym:N}$_{\text{p}} = \num{1,00}$) und dominiert aufgrund der einfachen Montage im Bereich mittlerer bis hoher Ströme. Zur Gewährleistung einer zentrierten Leiterführung bieten einige Hersteller spezielle Vorrichtungen zur Fixierung unterschiedlicher Schienengeometrien im Fensterausschnitt an.

Das zentrale Element der Übertragung bildet der Eisenkern. Ist dieser als geschlossener Toroid ohne Luftspalt ausgeführt, wird er als Ringkern bezeichnet. Der Kern besteht zur Minimierung der Übertragungsverluste aus einem ferromagnetischen Werkstoff mit hoher Permeabilität \gls{sym:mur} (siehe \ref{sec:magnetische_stoffeigenschaften}). Die magnetischen Eigenschaften des Kernmaterials sowie die Querschnittsfläche des Kerns bestimmen maßgeblich die Genauigkeit und das Sättigungsverhalten des Wandlers. Als Werkstoffe kommen üblicherweise Siliziumeisen, Nickeleisen oder nanokristalline Legierungen zum Einsatz~\cite[S.~63]{book-Technologie_Messwandler-Minkner}.
...
Im Gegensatz dazu dienen Schutzstromwandler der Ansteuerung von Schutzeinrichtungen wie beispielsweise Relais. Es muss gewährleistet sein, dass der Sekundärstrom auch bei einem Primärstrom \gls{sym:I_p} weit außerhalb des Nennbereichs proportional zum Primärleiterstrom verläuft. Der Eisenkern darf folglich nicht frühzeitig in Sättigung gehen, um eine zuverlässige Schutzauslösung im Fehlerfall sicherzustellen~\cite[Kap.~2.2]{book-Einmaleins_Stromwandler-Redur}.


\subsubsection{Kompensationswicklungen}
\label{sec:kompensationswicklungen}

Kompensationswicklungen dienen primär der Minimierung zweier signifikanter Störeinflüsse bei Messstromwandlern. Hierbei handelt es sich zum einen um den Lagefehler, welcher durch eine exzentrische Positionierung des Primärleiters hervorgerufen wird (siehe Abbildung~\ref{pic:schema_kupferschiene_pos}), und zum anderen um die Einwirkung von Fremdfeldern. Die physikalischen Grundlagen des letztgenannten Aspekts werden in Abschnitt~\ref{sec:stoerfelder_schaltanlagen} detailliert dargelegt.
...
Die Teilwicklungen $W_{1}$ bis $W_{4}$ sind symmetrisch über den Umfang des Eisenkerns verteilt. Technisch wird dies durch zusätzliche Teilwicklungen realisiert, die zur Wicklung $W_0$ parallel geschaltet sind. Die Abbildung stellt lediglich die allgemeine Kompensationswicklungstechnik dar. Da Hersteller in der Praxis oft individuelle und teils proprietäre Wicklungsdesigns einsetzen, kann die tatsächliche technische Ausführung von dieser schematischen Darstellung abweichen.

Parallelschaltung ermöglicht das Fließen von Ausgleichsströmen zwischen den Segmenten zur Kompensation lokaler Sättigungserscheinungen und Asymmetrien im magnetischen Fluss \gls{sym:Phi}~\cite[S.~77]{book-Technologie_Messwandler-Minkner}.


\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{03_Ressourcen/zeichnungen/kupferschiene_positionierung_vergleich.drawio.pdf}
    \caption{Schematische Darstellung der zentrischen und exzentrischen Positionierung der Kupferschiene}
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccc}
        \toprule
        Genauigkeits- & \multicolumn{4}{c}{Übersetzungsmessabweichung $\varepsilon\,\pm$ \SI{}{\percent}}                                             \\
...
Ein wesentliches Kriterium ist die Übersetzungsmessabweichung \gls{sym:epsilon} als prozentuale Abweichung des Sekundärstroms \gls{sym:I_sec} vom idealen, durch das Übersetzungsverhältnis bestimmten Sollwert. Dieser sogenannte Stromfehler beschreibt somit die Amplitudenabweichung zwischen tatsächlichem und ideal transformiertem Strom (siehe Tabelle~\ref{tab:stromfehler})~\cite[S.~21]{norm-DIN_EN_61869_2-Messwandler_Teil2-DIN}. Tabelle~\ref{tab:fehlwinkel} fasst die zulässigen Fehlwinkel für die Klassen \num{0,10} bis \num{1,00} zusammen.
...
Im Rahmen dieser Arbeit wird zur Bewertung der Fremdfeldeinflüsse ausschließlich die Übersetzungsmessabweichung \gls{sym:epsilon} herangezogen, da die beobachteten Effekte primär als amplitudenbezogene Sättigungserscheinungen des Kerns auftreten. Eine experimentelle Bestimmung des Fehlwinkels \gls{sym:deltaphi} erfolgt nicht, da die Messkette und die Auswertelogik auf stationäre Effektivwert- bzw. Mittelwertgrößen ausgelegt sind und der Fokus auf der normativen Einhaltung der Amplitudenfehlergrenzen liegt. Die normativen Grenzwerte für \gls{sym:deltaphi} werden im Folgenden der Vollständigkeit halber angegeben. Eine Erweiterung um eine Fehlwinkelanalyse wird im Ausblick adressiert.


\subsubsection{Ersatzschaltbild eines Messstromwandlers}
\label{sec:ersatzschaltbild}

Ein Messstromwandler basiert physikalisch auf dem Wirkprinzip des Transformators. Sein Übertragungsverhalten lässt sich daher mithilfe eines Transformator-Ersatzschaltbildes beschreiben. In der Darstellung gemäß Abbildung~\ref{esb:wandler_vollständig} sind alle primärseitigen Größen auf die Sekundärseite transformiert.

Hierbei repräsentieren \gls{sym:R}$'_{p}$ den ohmschen Widerstand und $L'_{p}$ die Streuinduktivität der Primärseite. Die Sekundärseite wird durch den Wicklungswiderstand \gls{sym:R}$_{s}$ und die Streuinduktivität $L_{s}$ modelliert. Der Querzweig, bestehend aus dem Eisenverlustwiderstand \gls{sym:R}$_{\text{Fe}}$ und der Hauptinduktivität $L_{\text{H}}$, bildet den Eisenkern ab, während \gls{sym:R}$_{\text{B}}$ und $L_{\text{B}}$ die externe Bürde repräsentieren.
...
Da die Permeabilität ferromagnetischer Materialien nichtlinear von der Feldstärke \gls{sym:H} abhängt, geht das Material bei hohen Feldstärken in den Sättigungsbereich über. Die daraus resultierende Abnahme der wirksamen Permeabilität \gls{sym:mur} sowie die Konsequenzen für Messstromwandler werden im folgenden Kapitel detailliert hergeleitet und quantifiziert. Die Übertragungsgenauigkeit nimmt dadurch bei hohen Primärströmen \gls{sym:I_p} und bei überlagerten magnetischen Fremdfeldern deutlich ab~\cite[Kap.~11]{book-Magnetisches_Feld_Moeller-Harriehausen}\cite[S.~73]{book-Technologie_Messwandler-Minkner}.

\subsubsection{Hysterese und reales Verhalten}
\label{sec:hysterese-und-reales_verhalten}

Ferromagnetische Werkstoffe zeichnen sich durch eine nichtlineare Magnetisierungskennlinie aus, deren physikalische Ursache in der Domänenstruktur des Materials begründet liegt. Im Kristallgefüge bilden sich mikroskopische Bereiche, die als magnetische Domänen oder \textit{Weißsche Bezirke} bezeichnet werden. In Abbildung~\ref{fig:weiss_bezirke} sind diese Domänen schematisch als pastellgelbe Teilflächen dargestellt. Innerhalb einer Domäne sind die atomaren magnetischen Momente überwiegend gleichgerichtet, was im Bild durch die nahezu parallel ausgerichteten Pfeile innerhalb einer Teilfläche angedeutet ist. Die räumlich begrenzten Übergangszonen zwischen benachbarten Domänen mit unterschiedlicher Magnetisierungsrichtung werden als Bloch-Wände bezeichnet~\cite[Kap.~11]{book-Magnetisches_Feld_Moeller-Harriehausen}.
...
Die Neukurve beschreibt den initialen Magnetisierungsvorgang eines unmagnetisierten Materials bis zur Sättigung. Bei anschließender Reduktion der Feldstärke \gls{sym:H} verläuft die Flussdichte \gls{sym:B} entlang einer Hystereseschleife und kehrt nicht zum Nullpunkt zurück. Ursächlich hierfür sind irreversible Verschiebungen der Bloch-Wände, die durch Gitterdefekte im Kristallgefüge behindert werden. Wenn die Feldstärke \gls{sym:H} auf \num{0} zurückgeführt wird, bleibt die Remanenzflussdichte \gls{sym:Br} bestehen. Für eine vollständige Entmagnetisierung ist eine entgegengesetzt gerichtete Feldstärke erforderlich, deren Betrag als Koerzitivfeldstärke \gls{sym:Hc} bezeichnet wird~\cite[Kap.~11]{book-Magnetisches_Feld_Moeller-Harriehausen}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{03_Ressourcen/simulation/hysterese/hysterese_kurve_final_colors.pdf}
    \caption{Schematische Darstellung von Neukurve und Hystereseschleife im $B$-$H$-Diagramm \cite{website-Magnetisierungskurve-Mietke}}
...
Die Messgenauigkeit von Stromwandlern im Betrieb wird maßgeblich von externen und systembedingten Faktoren beeinflusst. Wesentliche systembedingte Einflüsse sind eine fehlerhafte Bürdenbeschaltung und die geometrische Positionierung des Primärleiters, während magnetische Fremdfelder als externer Störeinfluss wirken~\cite[S.~65]{book-Technologie_Messwandler-Minkner}.

\pagebreak[4]
\medskip
\noindent\textbf{Einfluss der Bürde}\par
\label{par:einfluss_buerde}

Die Impedanz der Bürde bestimmt das Betriebsverhalten des Stromwandlers maßgeblich. Wie im vereinfachten Ersatzschaltbild (Abbildung~\ref{esb:wandler_vereinfacht}) dargestellt, setzt sich der Sekundärkreis aus der Innenimpedanz der Wicklung und der externen Bürde zusammen. Eine Vergrößerung des Gesamtimpedanzbetrags im Sekundärkreis erfordert eine höhere induzierte Spannung \gls{sym:Uo} zur Aufrechterhaltung des eingeprägten Sekundärstroms \gls{sym:I_sec}~\cite[S.~65--67]{book-Technologie_Messwandler-Minkner}.
...
Durch Einsetzten von Gleichung \eqref{eq:spannung_impedanz} in \eqref{eq:trafo_hauptgleichung} ergibt sich für den magnetischen Fluss \gls{sym:Phi}:
\begin{equation}
    \Phi
...
\pagebreak[1]
\medskip
\noindent\textbf{Magnetische Fremdfelder}\par
...
Die Variablen sind definiert als induzierte magnetische Fremdflussdichte \gls{sym:B_fremd} (in \unit{T}), Primärstrom \gls{sym:I_p} (in \unit{A}), äußerer Kernradius \gls{sym:R} (in \unit{m}), Leiterbreite \gls{sym:W} (in \unit{m}), Eisenquerschnitt \gls{sym:A_Fe} (in \unit{m^2}) und Außenphasenabstand \gls{sym:D} (in \unit{m}).
...
Ein größerer Kernquerschnitt \gls{sym:A_Fe} kompensiert die höhere Flussaufnahme, sodass der Anstieg der Flussdichte gering bleibt. Der absolute Fremdfluss \gls{sym:Phi_fremd} ist dabei unabhängig vom Querschnitt:
...
\pagebreak[1]
\medskip
\noindent\textbf{Exzentrische Positionierung der Kupferschiene}\par
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{03_Ressourcen/simulation/wandler_mit_kupferschine/wandler_kupferschine_1p-2.pdf}
    
    \vspace{0.5cm}
    
    \includegraphics[width=0.83\textwidth]{03_Ressourcen/zeichnungen/wandler_asymetrisch_mittlererverlauf.drawio.pdf}
    
    \caption{Oben, simulierter Verlauf des magnetischen Flusses entlang des mittleren Kernumfangs bei zentrierter und exzentrischer Primärleiterposition. Unten, Definition der Auswertestrecke entlang des mittleren Kernumfangs mit Startpunkt und Zuordnung der Viertelpunkte.}

    \label{pic:sim_exzentrische_montiert}
\end{figure}

\pagebreak[4]
Abbildung~\ref{pic:sim_exzentrische_montiert} zeigt den simulierten Verlauf des magnetischen Flusses \gls{sym:Phi} entlang des Kernumfangs. Die im Diagramm verwendete Länge ist dabei als Wegkoordinate entlang eines geschlossenen Pfads auf dem mittleren Kernradius definiert. Der Nullpunkt bei $0\,\mathrm{mm}$ liegt an der in der unteren Skizze markierten Referenzstelle, die Richtung der Zunahme entspricht der Pfeilrichtung. Die Markierungen bei $106\,\mathrm{mm}$, $212\,\mathrm{mm}$ und $318\,\mathrm{mm}$ entsprechen einer Viertel, Halb und Dreiviertelumrundung. Der Punkt \glqq Max\grqq{} kennzeichnet das Ende einer vollständigen Umrundung, also die gesamte mittlere Umfangslänge des Kerns.

Bei zentrierter Primärleiterlage verläuft \gls{sym:Phi} vergleichsweise gleichmäßig. Die Kurve zeigt nur eine moderate Welligkeit, die aus der diskreten Abtastung und den lokalen Material und Geometrieeinflüssen resultiert. Bei exzentrischer Positionierung entsteht dagegen eine ausgeprägte Asymmetrie entlang des Umfangs. In dem Umfangsbereich, der in der unteren Skizze dem primärleiternahen Sektor entspricht, steigt \gls{sym:Phi} deutlich an, während er auf der gegenüberliegenden Seite ein Minimum ausbildet. Dadurch vergrößert sich die Spannweite des Flussverlaufs erheblich, was eine lokal höhere Flussdichte und damit eine erhöhte Sättigungsneigung in leiternahen Kernbereichen begünstigt.

Diese Inhomogenität wirkt sich insbesondere dann auf die Messgenauigkeit aus, wenn die Sekundärwicklung \gls{sym:Ns} nicht exakt gleichmäßig über den Umfang verteilt ist oder wenn Teilbereiche des Kerns aufgrund der lokalen Vormagnetisierung in einen nichtlinearen Bereich geraten. In der Folge können sich die wirksame Übersetzung und die Phasenlage ortsabhängig verändern, was Messabweichungen begünstigt~\cite[S.~76]{book-Technologie_Messwandler-Minkner}.
...
Niederspannungsschaltanlagen dienen der Verteilung, Schaltung und Absicherung elektrischer Energie innerhalb eines Bemessungsspannungsbereichs von bis zu \SI{1000,00}{V}. Sie fungieren als zentrales Bindeglied zwischen der Energieeinspeisung, etwa über Transformatorenstationen, und den nachgeschalteten Verbrauchern. Aufgrund ihrer primären Funktion im Energiefluss sind diese Anlagen sowohl für hohe Dauerbetriebsströme als auch für signifikante mechanische und thermische Beanspruchungen infolge von Kurzschlussereignissen dimensioniert.

Der Aufbau folgt üblicherweise einem modularen Prinzip, bei dem mehrere Schaltschrankfelder über ein gemeinsames Hauptsammelschienensystem elektrisch gekoppelt sind. Dieses System ist zumeist im rückwärtigen oder oberen Bereich der Anlage positioniert und übernimmt die Funktion des zentralen Energieverteilers. Funktional lassen sich die Felder in drei wesentliche Kategorien unterteilen: Einspeisefelder beziehen die elektrische Energie aus der Quelle, führen sie der Hauptsammelschiene zu und beherbergen in der Regel die zentralen Schutz- sowie Messsysteme. Über die Abgangsfelder erfolgt hingegen die gezielte Energieverteilung an nachgelagerte Verbraucher oder Unterverteilungen, während Kupplungsfelder die galvanische Verbindung oder Trennung einzelner Sammelschienenabschnitte ermöglichen. Letztere dienen insbesondere dazu, die Versorgungssicherheit durch redundante Einspeisungen zu erhöhen oder notwendige Wartungsarbeiten im Teilbetrieb zu realisieren.

Ein kritisches Sicherheitsmerkmal der Konstruktion stellt die mechanische und thermische Kurzschlussfestigkeit dar. Die mechanische Struktur muss derart robust ausgeführt sein, dass sie selbst extremen Belastungen, beispielsweise einem Kurzschlussstrom von $I_k = \SI{100,00}{kA}$, ohne strukturelle Zerstörung standhält. Ein wesentlicher Aspekt des Personenschutzes ist die Störlichtbogenqualifikation. Im Fehlerfall eines internen Lichtbogens wird die schlagartig freigesetzte Druckenergie sowie die thermische Last kontrolliert über integrierte Druckentlastungsklappen abgeführt. Diese konstruktive Maßnahme verhindert ein Aufsprengen der Schaltschranktüren und schützt das Bedienpersonal vor den unmittelbaren Auswirkungen des Lichtbogens, während gleichzeitig ein beschleunigtes Erlöschen des Fehlers bewirkt wird.





\subsection{Messabweichung und normative Kenngrößen}
\label{sec:messabweichung_kenngroessen}
...
\begin{equation}
    \varepsilon = \frac{k_n \cdot I_{s} - I_{p}}{I_{p}} \cdot \SI{100}{\percent}
