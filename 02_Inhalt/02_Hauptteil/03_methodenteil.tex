...
Die vom Prüfstand exportierten Rohdaten liegen im \gls{csv}-Format vor und enthalten die zeitlichen Verläufe der Ströme \gls{sym:I_p} als Referenz sowie \gls{sym:I_sec} als Prüflingsstrom. Angesichts des Untersuchungsumfangs mit 31 Konfigurationen, jeweils zwei Messgeräten und drei Phasen ergeben sich insgesamt mehr als 180 auszuwertende Einzelmessreihen. Eine manuelle Verarbeitung in einer Tabellenkalkulation wäre aufgrund des hohen Zeitaufwands und der erhöhten Fehleranfälligkeit bei Übertragungen unzweckmäßig. Daher wurde eine spezialisierte Auswertungssoftware auf Basis von \textit{Python} entwickelt. Der erste Prozessschritt erfolgt im Modul \textit{„Manueller Rohdaten-Export“} (Abbildung~\ref{pic:manual_tagger}). Da die Datenerfassung am Prüfstand mit einer Abtastrate von \SI{2}{Hz} erfolgt, entstehen umfangreiche Zeitreihen. Der Rohdaten-Selektor identifiziert die aufgezeichneten Laststufen und isoliert die stationären Messbereiche. Dazu werden transiente Ein- und Ausschwingvorgänge aus dem Datensatz entfernt, sodass ausschließlich stabile Phasen in die weiterführende Genauigkeitsberechnung eingehen. Pro Laststufe werden exakt 560 Messpunkte ausgewertet, was einem Zeitfenster von 4 Minuten und 40 Sekunden entspricht. Obwohl das Plateau technisch fünf Minuten gehalten wird, werden die letzten 20 Sekunden als Sicherheitsabstand verworfen, damit keine Signalflanken des Abschaltvorgangs in die Berechnung einfließen. Die verbleibenden 560 Werte bilden eine statistisch belastbare Grundlage, und die selektierten Bereiche werden als sortierte Datensätze in der Datenbank persistiert.
