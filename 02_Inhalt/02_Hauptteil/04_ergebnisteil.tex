...
Die vorangehenden Abschnitte haben den Einfluss der Leitergeometrie auf die Messabweichungen \gls{sym:epsilon} für die untersuchten
Wandlerkonzepte technisch eingeordnet. Im Folgenden wird dieser Befund um eine wirtschaftliche Perspektive ergänzt, indem der aus
Gl.~\ref{eq:etotal} abgeleitete mittlere Fehlerkennwert \(\varepsilon_{\text{total}}\) über den Verbesserungsgrad \(\eta_{\text{geo}}\)
nach Gl.~\ref{eq:eta_geo} zwischen Parallel- und Dreiecksanordnung quantifiziert und den jeweiligen Marktpreisen gegenübergestellt wird.
Die Preisangaben dienen dabei als Orientierungswerte zur relativen Einordnung der Konzepte im Datensatz; sie ersetzen keine projektspezifische
Beschaffungs- oder Integrationskalkulation.

Positive Werte von \(\eta_{\text{geo}}\) bedeuten eine Reduktion des mittleren Fehlers durch die Dreiecksanordnung, negative Werte einen
Anstieg gegenüber der Parallelanordnung. Die Auswertung erfolgt getrennt nach den Betriebsbereichen Niederstrom
(\SIrange{5}{50}{\percent}), Nennstrom (\SIrange{80}{100}{\percent}) und Überlast (\SI{120}{\percent})
(vgl. Abschnitt~\ref{sec:einfluss_leitergeometrie}). Zusätzlich wird ein zusammenfassender Kennwert \emph{Gesamt} angegeben, der als
Mittelwert der Beträge der drei Bereichswerte definiert ist. Dieser Kennwert beschreibt primär die \emph{Stärke der Geometriewirkung}
(\emph{Geometriesensitivität}) und erlaubt damit eine komprimierte Aussage darüber, wie stark ein Wandlerkonzept auf die Leiteranordnung reagiert.
Eine eindeutige Richtung im Sinne einer generellen Überlegenheit einer Geometrie ist daraus nicht zwingend ableitbar, da dies vom dominanten
Betriebsbereich sowie vom jeweiligen Wandleraufbau abhängt.

Die nachfolgenden Abschnitte ordnen die Ergebnisse stromstärkenweise ein. Die Zusammenführung dieser Befunde mit Randbedingungen der
Integration (z.\,B. bauliche Freiheitsgrade, Retrofit-Fähigkeit, Streuung/Planbarkeit) erfolgt im Fazit (Kapitel~\ref{chap:fazit}).


\subsubsection{Wirtschaftlichkeit im Bereich \SIrange{2000}{2500}{A}}

Tabelle~\ref{tab:oeko_2000A_parallel_vs_dreieck} zeigt für \SI{2000}{A} ein heterogenes Bild: Einige Modelle weisen nur geringe, andere sehr
ausgeprägte Änderungen zwischen Parallel- und Dreiecksanordnung auf. Der \textit{MBS ASK101.4} zeigt in allen drei Lastbereichen positive
\(\eta_{\text{geo}}\)-Werte bei gleichzeitig niedrigem Preis. Innerhalb des Datensatzes ergibt sich damit eine Kombination aus moderater,
konsistenter Verbesserung und geringer Kostenbasis. Die Geometriesensitivität bleibt dabei im Vergleich zu stark reagierenden Modellen begrenzt.

Beim \textit{Celsa ALO 10030} fällt eine hohe Geometriesensitivität auf. Im Niederstrombereich ergibt sich im Datensatz eine gegenläufige
Tendenz (negatives \(\eta_{\text{geo}}\)), während im Nenn- und Überlastbereich deutliche positive Verbesserungen auftreten. Damit deutet das
Modellprofil darauf hin, dass die Leitergeometrie insbesondere bei hohen Lasten maßgeblich in das Fehlerbild eingreift. Diese Interpretation
ist konsistent mit den in Abschnitt~\ref{sec:analyse_messreihe_2000A} beschriebenen Sättigungs- bzw. Fremdfeldeffekten, die sich je nach
Anordnung unterschiedlich stark ausprägen.

Der kompensierte \textit{Celsa ALO 8030 K} zeigt insgesamt nur geringe Unterschiede zwischen den Geometrien, jedoch bei höherer Preisbasis.
Im Kontext dieses Datensatzes lässt sich der Mehrpreis daher weniger als „Geometriegewinn“ interpretieren, sondern eher als Beitrag zu einer
geringeren Abhängigkeit von geometrischen Randbedingungen. Diese Einordnung passt zur technischen Beobachtung der hohen Stabilität gegenüber
externen Feldern (vgl. Abschnitt~\ref{sec:analyse_messreihe_2000A}).

Die \glsentryshort{ffp}-Ausführung (\textit{Redur 13A1030.3ffp}) zeigt eine deutliche Richtungsabhängigkeit: Während im Niederstrombereich ein
leicht positives \(\eta_{\text{geo}}\) vorliegt, weisen Nenn- und Überlastbereich stark negative Werte auf. Im Datensatz bedeutet dies, dass
die Dreiecksanordnung in den für die Praxis häufig relevanten oberen Lastpunkten keine Verbesserung, sondern eine Verschlechterung gegenüber
der Parallelanordnung ausbildet. Die technischen Ursachen sind im Detail in Abschnitt~\ref{sec:analyse_messreihe_2000A} diskutiert
(Anordnung der Protektoren und fehlende Rückseitenabschirmung).
...
Für \SI{2500}{A} (Tabelle~\ref{tab:oeko_2500A_parallel_vs_dreieck}) zeigt der Standardwandler \textit{Celsa ALO 10030} in allen drei Bereichen
deutlich positive \(\eta_{\text{geo}}\)-Werte. In Kombination mit der niedrigen Preisbasis ergibt sich im Datensatz ein ausgeprägter
Kosten-Nutzen-Hebel der Geometrieänderung. Gleichzeitig ist aus der technischen Diskussion in Abschnitt~\ref{sec:analyse_messreihe_2500A}
ersichtlich, dass trotz Verbesserung im Hochlastbereich weiterhin Sättigungstendenzen verbleiben können; die ökonomische Bewertung ist daher
im Sinne eines relativen Vergleichs zwischen Geometrien zu lesen, nicht als Aussage über absolute Normkonformität in allen Punkten.

Der kompensierte \textit{Celsa ALO 10050 K} zeigt demgegenüber kleinere Verbesserungen bei höherem Preis. Im Datensatz spricht dies dafür,
dass der primäre Nutzen dieser Variante weniger in maximalen relativen Zugewinnen durch Geometrieoptimierung liegt, sondern in der insgesamt
geringeren Fehlerausprägung und der reduzierten Sensitivität gegenüber externen Feldbedingungen (vgl. Abschnitt~\ref{sec:analyse_messreihe_2500A}).
...
Im Hochstrombereich (Tabelle~\ref{tab:oeko_3000A_parallel_vs_dreieck} und Tabelle~\ref{tab:oeko_4000A_parallel_vs_dreieck}) wird im Datensatz
sichtbar, dass Standardwandler in Nenn- und Überlast besonders stark von der Dreiecksanordnung profitieren können. Für den
\textit{Celsa ALO 12070} liegen die \(\eta_{\text{geo}}\)-Werte bei \SI{3000}{A} und \SI{4000}{A} im Nenn- und Überlastbereich deutlich im
positiven Bereich, während der Niederstrombereich vergleichsweise schwach reagiert. Diese Charakteristik ist konsistent mit den technischen
Befunden aus Abschnitt~\ref{sec:analyse_messreihe_3000A} und Abschnitt~\ref{sec:analyse_messreihe_4000A}, wonach sich Sättigungseffekte
insbesondere bei hohen Lasten ausprägen und durch die Geometrie beeinflusst werden.

Der kompensierte \textit{Celsa ALO 12070 K} zeigt bei \SI{3000}{A} ein gemischtes Profil: Im Überlastbereich ist im Datensatz eine deutliche
Verbesserung durch die Dreiecksanordnung erkennbar, während Nieder- und Nennbereich negative Werte aufweisen. Ökonomisch fällt dies aufgrund
der hohen Preisbasis stärker ins Gewicht, wenn die Bewertung primär über den mittleren Zugewinn erfolgt. Gleichzeitig ist bei der Interpretation
zu berücksichtigen, dass negative \(\eta_{\text{geo}}\)-Werte hier lediglich eine Verschiebung zwischen zwei bereits kleinen Fehlern bedeuten
können (vgl. Bereichsanalysen in Abschnitt~\ref{sec:analyse_messreihe_3000A}).

Bei \SI{4000}{A} zeigt der kompensierte \textit{Celsa ALO 12070 K} im vorliegenden Datensatz in allen Bereichen negative Werte. Daraus folgt
für die reine \emph{Geometriewirkung} zunächst, dass die Dreiecksanordnung bei diesem Modell keinen zusätzlichen Verbesserungshebel darstellt.
Diese Aussage ist jedoch von der allgemeinen Robustheit des kompensierten Konzepts zu trennen: Die technischen Ergebnisse zeigen unabhängig
von \(\eta_{\text{geo}}\), dass die absoluten Fehlerwerte klein bleiben können (vgl. Abschnitt~\ref{sec:analyse_messreihe_4000A}).
...
Über alle Strombereiche zeigt der Datensatz zwei wiederkehrende Muster: Erstens treten die größten relativen Geometrieeffekte typischerweise
bei Standardwandlern im Nenn- und Überlastbereich auf, was sich mit den dort dominanten Sättigungs- und Fremdfeldeinflüssen deckt. Zweitens
weisen kompensierte und \glsentryshort{ffp}-basierte Konzepte häufig eine geringere Geometriesensitivität auf, wobei die Richtung des Effekts
vom konkreten Aufbau (z.\,B. Schirmorientierung) abhängen kann. In der reinen Kosten-\(\eta_{\text{geo}}\)-Gegenüberstellung wird damit die
Geometrieoptimierung als potenziell starker Hebel sichtbar, während Spezialtechnologien eher als Mittel zur Robustheit und zur Reduktion
geometrischer Randbedingtheit interpretierbar sind. Welche dieser Perspektiven in der Gesamtabwägung dominiert, hängt von den konkreten
Integrationsrandbedingungen und dem dominanten Betriebsbereich ab und wird im Fazit zusammengeführt (Kapitel~\ref{chap:fazit}).
