...
\begin{diagram}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{03_Ressourcen/diagramme/dia_3000A_buerde/dia_3000A_buerde-Oekonomie_Ranking.pdf}
...
Bei der Analyse des \textit{MBS~ASK129.10} (Details in Tabelle~\ref{tab:messergebnisse_mbs_5000A_mixed} im Anhang) offenbaren sich über alle drei Phasen hinweg signifikante Schwierigkeiten bei der Einhaltung der geforderten Messgenauigkeit, was sich insbesondere in der Parallelanordnung manifestiert. Während Phase L1 den Toleranzbereich bereits bei \SI{90}{\percent} des Nennstroms mit einer Abweichung \gls{sym:epsilon} von \num{-2,32}\,\% verlässt und im Überlastfall sogar auf \num{-6,72}\,\% abfällt, bricht die Genauigkeit der Phase L2 noch frühzeitiger ein. Bereits bei \SI{80}{\percent} \gls{sym:In} treten hier massive Messabweichungen von rund \num{-5,50}\,\% auf. Im Kontrast dazu zeigt sich Phase L3 in der gleichen Anordnung deutlich robuster und hält die Vorgaben weitestgehend ein, wobei der Nennpunkt mit \num{-1,06}\,\% nur marginal die Norm verfehlt.

Einen direkten technologischen Vergleich ermöglicht der \textit{Redur~20A1456.5FFP}, dessen Messwerte in Tabelle~\ref{tab:messergebnisse_redur_5000A_mixed} (Anhang) dokumentiert sind. Zwar zeigt dieser Wandler in der Parallelanordnung ein qualitativ ähnliches Verhalten wie das Modell von MBS, schneidet jedoch quantitativ präziser ab. Eine Anomalie offenbart sich jedoch beim Wechsel in die Dreieckskonfiguration: Während die Phasen L2 und L3 wie erwartet signifikant an Genauigkeit gewinnen, bleibt dieser Effekt bei L1 aus. 



Wie die Messwerte belegen, verschlechtert sich der Fehler bei L1 im Nennpunkt sogar leicht von \num{-1,85}\,\% in der Parallelanordnung auf \num{-2,00}\,\% im Dreieck. Dies deutet darauf hin, dass die räumliche Umpositionierung der Phase L2 im Dreieck zu einer ungünstigeren vektoriellen Addition der Streufelder im Bereich der L1-Kernsegmente führt, die durch die seitlichen Fremdfeldprotektoren nicht abgedeckt werden.

Als überraschend leistungsfähig erweist sich in diesem Hochstromszenario der Standardwandler \textit{Celsa~ALO~20060} (Tabelle~\ref{tab:messergebnisse_celsa20060_5000A_alle} im Anhang). Er übertrifft die beiden zuvor betrachteten Modelle deutlich. In der Parallelanordnung verbleiben die Außenleiter L1 und L3 über den gesamten Messbereich innerhalb der Genauigkeitsklasse \num{1}. Lediglich der mittlere Leiter L2 zeigt ab \SI{50}{\percent} des Nennstroms \gls{sym:In} Sättigungstendenzen und fällt bei Nennlast auf \num{-3,84}\,\% ab. Die Umstellung auf die Dreieckskonfiguration eliminiert diese Schwäche nahezu vollständig und stabilisiert die Messwerte auf einem bemerkenswert hohen Niveau. So verbessert sich der Fehler bei L2 um den Faktor 14 auf \num{-0,26}\,\% und auch L1 sowie L3 liefern mit \num{-0,31}\,\% und \num{-0,15}\,\% exzellente Ergebnisse.

Tabelle~\ref{tab:verbesserungsfaktor_alle_5000a} fasst die Wirksamkeit der geometrischen Optimierung für alle drei Wandlertypen zusammen. Der Faktor beschreibt das Verhältnis des Fehlers in Parallelanordnung zum Fehler in Dreieckskonfiguration (Werte $> \num{1,00}$ bedeuten eine Verbesserung).
...
\begin{table}[H]
    ...
    \begin{tabular}{llcccc}
        \toprule
        {Wandler}       & {Phase} & \SI{80}{\percent} & \SI{90}{\percent} & \SI{100}{\percent} & \SI{120}{\percent} \\
        \midrule
        \textit{MBS~ASK129.10}   & L1      & \num{2,46}        & \num{3,75}        & \num{2,88}         & \num{1,99}         \\
                        & L2      & \num{20,71}       & \num{13,14}       & \num{10,01}        & \num{7,59}         \\
                        & L3      & \num{0,93}        & \num{3,42}        & \num{27,69}        & \num{4,79}         \\
        \addlinespace
        \textit{Redur~20A1456}   & L1      & \num{0,79}        & \num{0,77}        & \num{0,93}         & \num{1,12}         \\
                        & L2      & \num{9,51}        & \num{6,68}        & \num{5,06}         & \num{3,69}         \\
                        & L3      & \num{1,36}        & \num{1,26}        & \num{0,94}         & \num{2,48}         \\
        \addlinespace
        \textit{Celsa~ALO~20060} & L1      & \num{5,93}        & \num{1,14}        & \num{1,11}         & \num{2,49}         \\
                        & L2      & \num{15,12}       & \num{10,68}       & \num{14,75}        & \num{32,51}        \\
                        & L3      & \num{1,06}        & \num{0,69}        & \num{1,10}         & \num{2,96}         \\
        \bottomrule
    \end{tabular}
...
Die Messreihe bei einem Nennstrom \gls{sym:In} von \SI{5000}{A} dient der Untersuchung des Grenzverhaltens der betrachteten Stromwandlerkonzepte unter extremen Betriebsbedingungen. Bereits bei Lastströmen deutlich unterhalb des Nennwertes verlassen mehrere Wandler die spezifizierte Genauigkeitsklasse, was auf eine systematische Überlastung der magnetischen Kerne durch Fremdfeldüberlagerung hindeutet.



Im Vergleich zu den Messreihen bei geringeren Nennströmen treten bei \SI{5000}{A} verstärkt nichtlineare Effekte auf, die sich in einer erhöhten Streuung der Messergebnisse sowie in einer ausgeprägten Phasenabhängigkeit äußern. Es ist davon auszugehen, dass im Hochstrombereich lokale Sättigungsphänomene auftreten, bei denen bereits geringe geometrische Asymmetrien, Fertigungstoleranzen oder Inhomogenitäten im Kernmaterial einen überproportionalen Einfluss auf das Messergebnis haben. Eine weitere Steigerung der Präzision wäre hier nur durch eine signifikante Vergrößerung des Eisenquerschnitts \gls{sym:A} oder den Einsatz hochpermeabler Kernmaterialien zu realisieren.
...
In der maximalen Laststufe von \SI{5000}{A} zeigt sich, dass nur noch großvolumige oder technologisch spezialisierte Wandler belastbare Ergebnisse liefern. 
...
Hier erreicht der Standardwandler \textit{Celsa~ALO~20060} in der Dreieckskonfiguration den besten Performance-Score, da sein massiver Eisenkern in Kombination mit der feldoptimierten Geometrie eine außergewöhnlich hohe Präzision ermöglicht, die den moderaten Mehrpreis gegenüber unterdimensionierten Modellen rechtfertigt.

% --- Cluster 3: Volumenbetrachtung ---
\subsubsection{Einfluss des Bauvolumens auf die Gesamtwirtschaftlichkeit}

Abschließend wird der Faktor Bauraum in die Bewertung einbezogen. In modernen Schaltanlagen korreliert das Volumen eines Bauteils oft direkt mit indirekten Folgekosten, wie etwa größeren Schaltschrankfeldern oder erhöhtem Kupfereinsatz für weite Schienenabstände.



Vergleicht man die volumen-gewichteten Rankings bei \SI{2000}{A} (Abbildung~\ref{dia:2000A_ranking_volumen_plot}) und \SI{5000}{A} (Abbildung~\ref{dia:5000A_ranking_volumen_plot}), zeigt sich ein signifikanter Zielkonflikt: Kompakte Wandler sind oft günstiger im Preis-Volumen-Verhältnis, erkaufen sich diese Kompaktheit jedoch durch geringere Eisenquerschnitte \gls{sym:A} und damit früher einsetzende Sättigung. Dies belegt, dass eine reine Minimierung des Bauvolumens technisch kontraproduktiv sein kann, wenn dadurch die Messaufgabe gefährdet wird.
